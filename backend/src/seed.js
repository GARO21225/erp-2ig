 ...
 * Lance après : npx prisma db push
 */
...
async function seed() {
  ...
  for (const c of caisses) {
    await prisma.caisse.upsert({ where: { id: c.nom.toLowerCase().replace(/ /g, '_') },
      update: {}, create: { id: c.nom.toLowerCase().replace(/ /g, '_'), ...c } })
      .catch(() => {});
  }
  console.log('✅ Caisses');
  ...
  for (let i = 1; i <= 12; i++) {
    ...
    const cap = i === 7 ? 8 : i >= 11 ? 6 : [3,5,6].includes(i) ? 6 : 4;
    await prisma.tableRestaurant.upsert({
      where: { numero: i }, update: {}, create: { numero: i, capacite: cap, zone }
    });
  }
  console.log('✅ Tables restaurant (12)');

  // 3. Menu Yakro Grill (80 articles)
  const menuItems = [
  ...
  for (const item of menuItems) {
    try {
      await prisma.menuYakro.create({ data: item });
      menuCount++;
    } catch (e) { /* Déjà existant */ }
  }
  console.log(`✅ Menu Yakro Grill (${menuCount} articles)`);

  console.log('\n🎉 Seed terminé ! Vous pouvez maintenant créer le premier compte DG.');
  console.log('→ POST /api/auth/register avec role="DG" et filiale="GROUPE"');
}
